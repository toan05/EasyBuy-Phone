    ...
    public class OrderController : Controller
    {
            ...
            if (response.Success)
            {
                ...
                if (!string.IsNullOrEmpty(voucherCode))
                {
                    ...
                    if (voucher != null)
                    {
                        ...
                        // Decorator áp dụng Voucher và Seasonal (nếu có)
                        var discountCalculator = new SeasonalDiscountDecorator(
                            new VoucherDiscountDecorator(
                                new BaseDiscountCalculator()));

                        discount = discountCalculator.CalculateDiscount(total, voucher);
        ...
        public async Task<IActionResult> PaymentCallbackMomo()
        {
            try
            {
                ...
                if (momoResponse != null && !string.IsNullOrEmpty(momoResponse.OrderId) && momoResponse.OrderId == momoOrderId)
                {
