    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(string name, string password, bool rememberMe = false)
        {
            try
            {
                ...
                if (isPasswordValid)
                {
                    ...
                    // Thiết lập session để hiển thị thông tin khách hàng trên giao diện
                    HttpContext.Session.SetInt32("UserID", user.UserId);
                    HttpContext.Session.SetString("Phone", user.Phone ?? string.Empty);
        ...
        public async Task<IActionResult> Logout()
        {
            try
            {
                ...
                // Xoá session thông tin khách hàng
